...
- range(10) should do the same thing as range(w10, false, false)
...
const range = function(count, skipZero, descending) {
  ...
  let comparator = (descending, index, count) => {
    if (descending) {
  ...
  let rtnArr = [];
  for (let index = ((descending) ? newCount : startAt); comparator(descending, index, newCount); ((descending) ? index-- : index++)) {
